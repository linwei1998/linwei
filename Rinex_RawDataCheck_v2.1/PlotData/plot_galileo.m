function plot_galileo(OBS,fdcsvfile,signal_mask,range_mask,phase_mask,dcb_mask,noise_mask,pl_mask,mp_mask)
 ...
 %202309200W add average calculation
    ...
    if (isfield(OBS,'GALobs'))                                             %处理Galileo卫星数据
       ...
       for j=1:length(ind)
           ...
           osv=ind(j);
           LengthS=length(OBS.GALobs.C1(osv,:));
           P1=[];
           P2=[];
           L1=[];
           L2=[];
           ...
           if(isfield(OBS.GALobs,'C1') && isfield(OBS.GALobs,'P2') )
               for i=1:length(OBS.GALobs.C1(osv,:))
                   if(~isnan(OBS.GALobs.C1(osv,i))&&~isnan(OBS.GALobs.P2(osv,i))&&(OBS.GALobs.C1(osv,i)~=0)&&(OBS.GALobs.P2(osv,i)~=0))
                      dcb(i)=OBS.GALobs.P2(osv,i)-OBS.GALobs.C1(osv,i);
                   end   
                   if(isfield(OBS.GALobs,'L1') && isfield(OBS.GALobs,'L2'))
                      if((OBS.GALobs.C1(osv,i)~=0)&&(OBS.GALobs.P2(osv,i)~=0)&&(OBS.GALobs.L1(osv,i)~=0)&&(OBS.GALobs.L2(osv,i)~=0)&&...
                          ~isnan(OBS.GALobs.C1(osv,i))&&~isnan(OBS.GALobs.P2(osv,i))&&~isnan(OBS.GALobs.L1(osv,i))&&~isnan(OBS.GALobs.L2(osv,i)))
                         mp1(i)=OBS.GALobs.C1(osv,i)-((1+aph)/(aph-1))*lamda_1*OBS.GALobs.L1(osv,i)+2.0/(aph-1)*lamda_2*OBS.GALobs.L2(osv,i);
                         mp2(i)=OBS.GALobs.P2(osv,i)-2.0*aph/(aph-1)*lamda_1*OBS.GALobs.L1(osv,i)+(1+aph)/(aph-1)*lamda_2*OBS.GALobs.L2(osv,i);
                         DL1L2(i)=OBS.GALobs.L1(osv,i)*lamda_1-lamda_2*OBS.GALobs.L2(osv,i);
                      end 
                   end
               end
           end
           
           if(isfield(OBS.GALobs,'C1'))
               
               len=0;
               P1=OBS.GALobs.C1(osv,:);
               P1((P1==0))=nan;
               S1=OBS.GALobs.S1(osv,:);
               S1(S1==0)=nan;
...
%                    if noise_mask
                       figure
                       plot(ddddiffP1,'b.')
                       title(['E',num2str(osv),' L1 PR Qurd-Diff']);
                       grid on
                       xlabel('epoch')
                       ylabel('range:m')
%                    end
...
%                         P1_noise = sqrt(sum(ddddiffP1.*ddddiffP1,'omitnan')/(8*(len-1)));
                   ...
                   disp(['E',num2str(osv),' L1 pseudorange noise: ',num2str(P1_noise)]);
               end
           end
           
           if(isfield(OBS.GALobs,'L1'))
               
               len=0;
               L1=OBS.GALobs.L1(osv,:);
               L1((L1==0))=nan;
               ...
               if ~all(isnan(L1))
                   ...
                   if noise_mask
                       figure
                       plot(dddiffL1,'r+')
                       title(['E',num2str(osv),' L1 carrie phase accuracy']);
                       grid on
                       xlabel('epoch')
                       ylabel('range:cycle')
                   end
                   ...
                   disp(['E',num2str(osv),' L1 carrie phase noise: ',num2str(L1_noise)]);
               end
           end
           
           if(isfield(OBS.GALobs,'D1'))
               
               len=0;
               D1=OBS.GALobs.D1(osv,:);
               D1((D1==0))=nan;
               ...
               if ~all(isnan(D1))
                   ...
                   if noise_mask
                       figure
                       plot(dddiffD1,'y*')
                       title(['E',num2str(osv),' L1 doppler accuracy']);
                       grid on
                       xlabel('epoch')
                       ylabel('range:HZ')
                   end
                   ...
                   disp(['E',num2str(osv),' L1 doppler noise: ',num2str(D1_noise)]);
               end
           end
           
           if(isfield(OBS.GALobs,'P2'))
               
               len=0;
               P2=OBS.GALobs.P2(osv,:);
               P2((P2==0))=nan;
               S2=OBS.GALobs.S2(osv,:);
               S2(S2==0)=nan;
               ...
               if ~all(isnan(P2))
                   ...
                   plot(S2,'r')
                   if signaltype==2
                       title(['E',num2str(osv),' L2 CN0']);
                   else
                       title(['E',num2str(osv),' L5 CN0']);
                   end
                   grid on
                   xlabel('epoch')
                   ylabel('CN0:dBHZ')
...
%                    if noise_mask
                       ...
                       plot(ddddiffP2,'r.')
                       if signaltype==2
                           title(['E',num2str(osv),' L2 PR Qurd-Diff']);
                       else
                           title(['E',num2str(osv),' L5 PR Qurd-Diff']);
                       end
                       grid on
                       xlabel('epoch')
                       ylabel('range:m')
...
%                         P2_noise = sqrt(sum(dddiffP2.*dddiffP2,'omitnan')/(8*len-8));
                   ...
                   if signaltype==2
                       disp(['E',num2str(osv),' L2 pseudorange noise: ',num2str(P2_noise)]);
                   else
                       disp(['E',num2str(osv),' L5 pseudorange noise: ',num2str(P2_noise)]); 
                   end
               end
           end
           
           if(isfield(OBS.GALobs,'L2'))
               
               len=0;
               L2=OBS.GALobs.L2(osv,:);
               L2((L2==0))=nan;
               ...
               if ~all(isnan(L2))
                   ...
                   if noise_mask
                       ...
                       plot(dddiffL2,'g+')
                       if signaltype==2
                           title(['E',num2str(osv),' L2 carrie phase accuracy']);
                       else
                           title(['E',num2str(osv),' L5 carrie phase accuracy']);
                       end
                       grid on
                       xlabel('epoch')
                       ylabel('range:cycle')
                   ...
                   if signaltype==2
                       disp(['E',num2str(osv),' L2 carrie phase noise: ',num2str(L2_noise)]);
                   else
                       disp(['E',num2str(osv),' L5 carrie phase noise: ',num2str(L2_noise)]);
                   end
               end
           end
           
           if(isfield(OBS.GALobs,'D2'))
               
               len=0;
               D2=OBS.GALobs.D2(osv,:);
               D2((D2==0))=nan;
               ...
               if ~all(isnan(D2))
                   ...
                   if noise_mask
                       ...
                       plot(dddiffD2,'y*')
                       if signaltype==2
                           title(['E',num2str(osv),' L2 doppler accuracy']);
                       else
                           title(['E',num2str(osv),' L5 doppler accuracy']);
                       end
                       grid on
                       xlabel('epoch')
                       ylabel('range:HZ')
                   ...
                   if signaltype==2
                       disp(['E',num2str(osv),' L2 doppler noise: ',num2str(D2_noise)]);
                   else
                       disp(['E',num2str(osv),' L5 doppler noise: ',num2str(D2_noise)]);
                   end
               end
           end
           
           if(isfield(OBS.GALobs,'C1') && isfield(OBS.GALobs,'L1'))
              DiffPL1=OBS.GALobs.C1(osv,:)-OBS.GALobs.L1(osv,:)*lamda_1;
              for i=1:length(OBS.GALobs.C1(osv,:))-1
              ...
              disp(['E',num2str(osv), ':Mean DDPL1=',num2str(mean_ddpl1),'m  ','Std DDPL1=',num2str(std_ddpl1),'m']);
           end
           
           if(isfield(OBS.GALobs,'P2') && isfield(OBS.GALobs,'L2'))
              DiffPL2=OBS.GALobs.P2(osv,:)-OBS.GALobs.L2(osv,:)*lamda_2; 
              DiffPL2(DiffPL2==0)=nan;
              for i=1:length(OBS.GALobs.P2(osv,:))-1
              ...
              if signaltype==2
                   disp(['E',num2str(osv), ':Mean DDPL2=',num2str(mean_ddpl2),'m  ','Std DDPL2=',num2str(std_ddpl2),'m']);
              else 
                   disp(['E',num2str(osv), ':Mean DDPL5=',num2str(mean_ddpl2),'m  ','Std DDPL5=',num2str(std_ddpl2),'m']);
              end
           end
...
%                        ylim([-0.05,0.05])
                       if signaltype==2
                           title(['E',num2str(osv),' DDGF L1L2']);
                       else
                           title(['E',num2str(osv),' DDGF L1L5']);
                       end
                  end
            end
           ...
           if range_mask
               figure
               plot(P1,'b*')
               title(['E',num2str(osv),' Pseudo range']);
               grid on
               xlabel('epoch')
               ylabel('range:m')
               hold on
               ...
               figure
               plot(diffP1,'b*')
               title(['E',num2str(osv),' Pseudo range diff']);
               grid on
               xlabel('epoch')
               ylabel('range:m')
               hold on
           ...
           if phase_mask
               figure
               plot(L1,'b*')
               title(['E',num2str(osv),' Carrier phase']);
               grid on
               xlabel('epoch')
               ylabel('range:m')
               hold on
           ...
           if (pl_mask && ~isempty(DDPL1))
               figure
               plot(DDPL1,'b.')
               title(['E',num2str(osv),' code - phase']);
               grid on
               xlabel('epoch')
               ylabel('unit:m')
               if~isempty(DDPL2)
           ...
           if (dcb_mask &&~all(isnan(dcb)))
               figure
               plot(dcb)
               title(['E',num2str(osv),' difference code bias']);
               grid on
               xlabel('epoch')
               ylabel('unit:m')
           end
           ...
           if (~all(isnan(mp1)) && ~all(isnan(mp2)))
               ...
               if signaltype==2
                   disp(['E',num2str(osv), ':MP12=',num2str(std_mp1),'m  ','MP21=',num2str(std_mp2),'m']);
               else 
                   disp(['E',num2str(osv), ':MP15=',num2str(std_mp1),'m  ','MP51=',num2str(std_mp2),'m']);
               end
               
               if( mp_mask && ~all(isnan(mp1)) && ~all(isnan(mp2)))
                   figure
                   plot(mp1,'b.')
                   title(['E',num2str(osv),' MultipulPath']);
                   grid on
                   xlabel('epoch')
                   ylabel('range:m')
                   hold on
           ...
           fprintf(fdcsvfile,'\nE%02d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.4f,%.4f,%.3f,%.3f,%.3f,%.3f,%.3f',...
        ...
        fprintf(fdcsvfile,'\nE,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.4f,%.4f,%.3f,%.3f,%.3f,%.3f,%.3f',...
